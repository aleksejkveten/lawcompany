...
в сущность phone и email добавить поле notForMessage (по умолчанию false), добавить это поле на все crud Company.
...
Для модели Task добавь свойство deadline, добавь его в crud и в api эндпоинты.

т.е. будет строка вида 1) Общий (или иное имя) - Емаилы, Телефоны с галкой notForMessage (toggle без label), при этом рядом с каждым именем contactPerson, телефоном или емаил должна быть маленькая иконка редактирования, по которому открывается небольшое окно рядом (absolute) с кнопкой сохранить.  Для этого нужно переделать и эндпоинты, возможно, для сохранения создать новые.
...
т.е. будет строка вида Номер позиции Общий (или иное имя) - Емаилы, Телефоны  с  notForMessage (toggle без label), при этом рядом с каждым именем contactPerson, телефоном или емаил должна быть маленькая иконка редактирования, по которому открывается небольшое окно рядом (absolute) с кнопкой сохранить. Для этого нужно переделать и эндпоинты, возможно, для сохранения создать новые.
...
При отправке смс - проверять, соответсвует ли телефон мобильному (он соответствует, если начинается с +37529 +37525 +37544, 37533) ставить в очередь (bullmq) на отправку с random временем от +5 секунд до + 5 минут.
...
Если запланировать цепочку, то планируем смс и емаил. Емаил планируем также, как и выше (на дни вперед планируем с 09.00 утра и дальше), смс на последующие дни ставить на 10:00 утра все. Планирование писем только на дневное время и будние дни!!!

Если больше 1 контакта, то отправлять по всем контактам, за исключением isDeleted и notForMessage.

Все сообщения сразу сохранятся в SentSms или SentEmail.
Соответственно для этой задачи необходимо не только ставить задачи в очередь, но и написать обработчики отправки: для email - через yandex, дополнительный - через api (закоментировать этот код), для smsby - через api (будут одиночные sms и в 10:00 утра могут быть множественные письма - bulksms), также при отправке sms методом bulk в 10:00 необходимо запрашивать статусы ранее направленных sms. @nuxt-app\server\utils\sms-smsby.ts @nuxt-app\server\utils\email-yandex.ts @nuxt-app\server\utils\email-yandex.ts
