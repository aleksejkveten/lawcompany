    ...
    <div class="p-6">
      ...
      <div v-else class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <h2 class="text-lg font-medium text-gray-900 mb-4">Информация о SMS</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label class="block text-sm font-medium text-gray-500 mb-1">Номер телефона</label>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="bg-gray-50 border border-gray-300 rounded-md p-4">
            <pre class="whitespace-pre-wrap text-sm text-gray-900">{{ message.content }}</pre>
          </div>
          <div class="mt-2 text-sm text-gray-500">
            Длина: {{ message.content?.length || 0 }} символов
          </div>
        </div>
...
const fetchMessage = async () => {
  try {
    loading.value = true
    const { data } = await $fetch(`/api/panel/sent-sms/${route.params.id}`)
    message.value = data
  } catch (error) {
