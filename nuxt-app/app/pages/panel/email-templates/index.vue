  ...
  <div class="p-6">
    <div class="mb-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Шаблоны Email</h1>
        <div class="flex items-center space-x-3">
          ...
          <NuxtLink to="/panel/email-templates/new"
              ...
              <tr v-for="template in templates" :key="template.id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex justify-end space-x-2">
                    <NuxtLink
                      :to="`/panel/email-templates/${template.id}/detail`"
                      class="text-blue-600 hover:text-blue-900"
                    >
                      Просмотр
                    </NuxtLink>
                    <NuxtLink
                      :to="`/panel/email-templates/${template.id}/edit`"
...
const fetchTemplates = async () => {
  try {
    loading.value = true
    const response = await $fetch('/api/panel/email-templates', {
      query: {
...
const deleteTemplate = async () => {
  ...
  deleting.value = true
  try {
    await $fetch(`/api/panel/email-templates/${templateToDelete.value.id}`, {
      method: 'DELETE'
    })
