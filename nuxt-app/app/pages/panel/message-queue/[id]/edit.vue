...
const fetchJob = async () => {
  try {
    ...
    form.value = {
      type: data.data.type || '',
      templateId: data.data.templateId?.toString() || '',
      companyId: data.data.companyId?.toString() || '',
      scheduledAt: data.data.scheduledAt ? new Date(data.data.scheduledAt).toISOString().slice(0, 16) : '',
      key: data.data.key || '',
...
// Load data on mount
onMounted(async () => {
  await Promise.all([fetchJob(), loadData()])
})
</script>
