...
if (window.courtDataCollectorScriptLoaded) {
    // Script already loaded
} else {
    window.courtDataCollectorScriptLoaded = true;
    ...
    if (typeof window.CourtDataCollector === 'undefined') {
        ...
        function extractCourtNameFromContext(table) {
            try {
                // First, try to find court name in panel heading above the table
                let panel = table.closest('.panel');
                if (panel) {
                    const panelHeading = panel.querySelector('.panel-heading, .panel-title');
                    if (panelHeading) {
                        const headingText = panelHeading.textContent.trim();
                        if (isCourtName(headingText)) {
                            return headingText;
                        }
                    }
                }

                // Search in previous siblings (more comprehensive search)
                let current = table.previousElementSibling;
                let searchDepth = 0;

                while (current && searchDepth < 15) {
                    const text = current.textContent.trim();
                    if (isCourtName(text)) {
                        return text;
                    }
                    // Also check child elements for court names
                    const childElements = current.querySelectorAll('*');
                    for (let child of childElements) {
                        const childText = child.textContent.trim();
                        if (isCourtName(childText)) {
                            return childText;
                        }
                    }
                ...
                // Search in parent elements (more comprehensive)
                ...
                while (parent && searchDepth < 8) {
                    ...
                    // Check all text content in parent and its children
                    const allText = parent.innerText || parent.textContent || '';
                    if (isCourtName(allText)) {
                        return allText.trim();
                    }
                ...
                // Try to find in h1-h6 headers anywhere on the page
                const headers = document.querySelectorAll('h1, h2, h3, h4, h5, h6, .court-name, [class*="court"], [class*="суд"]');
                for (let header of headers) {
                    const text = header.textContent.trim();
                    if (isCourtName(text)) {
                        return text;
                    }
                    // Check inner elements too
                    const innerElements = header.querySelectorAll('*');
                    for (let inner of innerElements) {
                        const innerText = inner.textContent.trim();
                        if (isCourtName(innerText)) {
                            return innerText;
                        }
                    }
                }

                // Search for any element containing court keywords
                const allElements = document.querySelectorAll('*');
                for (let element of allElements) {
                    const text = element.textContent.trim();
                    if (isCourtName(text) && text.length < 100) { // Avoid very long texts
                        return text;
                    }
                }
                ...
                // Try to extract from URL if it contains court info
                const url = window.location.href;
                if (url.includes('minsk') || url.includes('минск')) {
                    return 'Экономический суд г. Минска';
                }
